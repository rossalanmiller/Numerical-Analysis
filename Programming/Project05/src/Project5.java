...
public class Project5
{
	...
	{
		return Math.pow(Math.E, x) + Math.pow(2.0, -x) + (2.0*Math.cos(x)) - 6.0;
	}
	//f prime
	public static double fp(double x)
	{//e^x+(2^(-x) )(-ln⁡(2) )-2sin⁡(x)
		return Math.pow(Math.E, x) - (Math.pow(2.0, -x) * Math.log(2)) - (2.0*Math.sin(x));
	}
	//Newton's method
	...
	public static void printHeader(double E, double P0)
	{
		...
		header += String.format("%12s|", "    Pn-1      Pn-2    ");
